            ...
            if (backGround.isFlag() && this.x >= 500) {
                ...
                for(int i = 0; i < backGround.getEnemyList().size(); i++){
                    ...
                    if(e.getY() == this.y + 20 && (e.getX() - 25 <= this.x && e.getX() + 35 >= this.x)){
                        if (e.getType() == 1){//1号敌人是蘑菇怪敌人
                            e.death();
                            score += 2;
                            upTime = 3;//踩到后马里奥上升一小段
                            ySpeed = -10;
                    ...
                    if ((e.getX() + 35 > this.x && e.getX() - 25 < this.x) && (e.getY() + 35 > this.y && e.getY() - 20 < this.y)) {//判断马里奥是否碰到了敌人}
                        death();//马里奥死亡
                    }
                }
